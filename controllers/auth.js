...
exports.preSignup = (req, res) => {
    ...
    User.findOne({ email: email.toLowerCase() }, (err, user) => {
        ...
        const emailData = {
            from: process.env.EMAIL_FROM_ACCOUNT_ACTIVATION, // MAKE SURE THIS EMAIL IS YOUR GMAIL FOR WHICH YOU GENERATED APP PASSWORD
        ...
        sendEmailWithNodemailer(req, res, emailData, undefined, email);
    });
};
    ...
    if (token) {
        jwt.verify(token, process.env.JWT_ACCOUNT_ACTIVATION, (err, decode) => {
            if (err) {
                return res
                    .status(401)
                    .json({ error: "Expired link. Signup again" });
            }
